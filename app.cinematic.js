...
(function risk(){
  ...
  let cur=max;
  const step=()=>{cur-=(cur-target)*0.1;if(Math.abs(cur-target)<0.5)cur=target;arc.style.strokeDashoffset=cur;if(cur!==target)requestAnimationFrame(step);};
  const svg=document.querySelector('.risk-circle svg');
  const defs=document.createElementNS('http://www.w3.org/2000/svg','defs');
  const grad=document.createElementNS('http://www.w3.org/2000/svg','linearGradient');
  grad.setAttribute('id','goldGrad');grad.innerHTML=`<stop offset="0%" stop-color="#4a3b14"/><stop offset="45%" stop-color="#c6a442"/><stop offset="55%" stop-color="#ffea94"/><stop offset="65%" stop-color="#bf9631"/><stop offset="100%" stop-color="#4a3b14"/>`;
